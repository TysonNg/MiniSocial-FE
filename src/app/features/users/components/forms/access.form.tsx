...
export function FormLogin() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {message.length > 0 && <div className="flex flex-col mt-5">{message.map((mess,index) => {
        return(
            <div key={index} className={`${mess === 'Login successfully!'? 'text-green-400': 'text-red-400'} font-bold`}>{mess}</div>
        )
      })}</div>}
    ...
    const handleSubmit = async (e : React.FormEvent) => {
      ...
      return (
        <form onSubmit={handleSubmit}>
          ...
          {message.length > 0 && message[0]  !== 'Register successfully!' && message[0] !== '' && (
            <p className="mt-5 text-red-400 font-bold w-1/2 mx-auto text-left">Error:</p>
          )}
          {message.length > 0 && <div className="flex flex-col mt-3 mx-auto w-1/2 text-left">{message.map((mess,index) => {
            return(
                <div key={index} className={`${mess === 'Register successfully!'? 'text-green-400': 'text-red-400'} font-bold`}>
                    {mess}    
                </div>
            )
          })}</div>}
